...
async def main():
    ...
    storage_state_path = "data/yad2_storage_state.json"
    ...
    async with async_playwright() as p:
        ...
        # Prepare context options matching Yad2PlaywrightScraper
        desktop_user_agents = [
        ...
        if os.path.exists(storage_state_path):
            print("Loading existing yad2_storage_state.json context state...")
        ...
        print("Navigating to Yad2 homepage...")
        await page.goto("https://www.yad2.co.il/", wait_until="domcontentloaded")

        print("\n--> PLEASE SOLVE THE CAPTCHA NOW in the opened browser window.")
        print("Wait until you are fully inside Yad2 and can see the homepage content.")
        print("Once the CAPTCHA is solved, press ENTER in this terminal to save the session state...")
        ...
        scp_cmd_str = f"scp -i {REMOTE_SSH_KEY} -o StrictHostKeyChecking=no {storage_state_path} {REMOTE_USER}@{REMOTE_HOST}:{REMOTE_PATH}data/"

        print("\n====================================================")
        choice = await loop.run_in_executor(
            None,
            lambda: input("Do you want to automatically copy the saved Yad2 session state to the remote server? (Y/n): ").strip().lower()
        )

        if choice in ("", "y", "yes"):
            if not shutil.which("scp"):
                print("\nError: 'scp' command line utility not found in PATH.")
                print("Please copy the file manually using the command below:")
                print(scp_cmd_str)
            else:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        print("Successfully copied Yad2 session state to the remote server!")
                    else:
                        print(f"SCP transfer failed with exit code {result.returncode}.")
                        print(f"Error output:\n{result.stderr}")
                        print("\nPlease copy the file manually using the command below:")
                        print(scp_cmd_str)
                except Exception as e:
